...
public final class QueueFile implements Closeable, Iterable<byte[]> {
    ...
    private QueueFile(File file, RandomAccessFile raf, boolean zero, boolean forceLegacy) throws IOException {
        this.file = file;
        this.raf = raf;
        this.zero = zero;
    ...
    private Element readElement(long position) throws IOException {
        ...
        ringRead(position, buffer, Element.HEADER_LENGTH);
    ...
    private byte[] getData(Element element) throws IOException {
        ...
        ringRead(element.position + Element.HEADER_LENGTH, data, length);
        return data;
    }
    ...
    private void remove(int n) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ringRead(newFirstPosition, buffer, Element.HEADER_LENGTH);
